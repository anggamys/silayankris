			...
			<form action="{{ route('admin.sekolah.update', $sekolah) }}" method="POST">
				...
				<div class="mb-3">
					...
					<input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" name="nama"
						value="{{ old('nama', $sekolah->nama) }}" required autofocus placeholder="Masukkan nama Sekolah">
					@error('nama')
				...
				<div class="mb-3">
					...
					<input type="text" class="form-control @error('alamat') is-invalid @enderror" id="alamat" name="alamat"
						value="{{ old('alamat', $sekolah->alamat) }}" required placeholder="Masukkan alamat Institusi">
					@error('alamat')
						<div class="invalid-feedback">{{ $message }}</div>
					@enderror
				</div>
